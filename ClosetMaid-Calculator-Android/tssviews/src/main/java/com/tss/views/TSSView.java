...
public class TSSView extends View implements OnGlobalLayoutListener, OnPreDrawListener{


    public TSSView(Context context, AttributeSet attrs) {
        super(context, attrs);
    ...
    private int auxHeight=-1;
    private int auxWidth=-1;
    ...
    private void setLeft(){
        if(x!=-1){
            l=(int)(x*getParentVirtualWidth());
        }
    }

    private void setTop(){
        if(y!=-1){
            t=(int)(y*getParentVirtualHeight());
        }
    }
    ...
    private void setWidth(){
        if(width!=-1){
            w=(int)(width*getParentVirtualWidth());
        }else{
    ...
    private void setHeight(){
        if(height!=-1){
            h=(int)(height*getParentVirtualHeight());
        }else{
    ...
    private int getParentVirtualWidth(){
        View parent=(View)this.getParent();
        int width=parent.getMeasuredWidth();
        return width;
    }

    private int getParentVirtualHeight(){
        View parent=(View)this.getParent();
    ...
    public boolean onPreDraw() {
        ...
        //			Bitmap scaled = Bitmap.createScaledBitmap(src, getWidth(), getHeight(), true);
        //			setBackgroundDrawable(new BitmapDrawable(getResources(), scaled));
        //		}
